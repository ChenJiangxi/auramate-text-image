...
「每张写满字、别大片留白」这条规则我在 skill 里写过三次，然后**连续违反了二十次**。
2026-08-10 一量：20 张正文页**全部**没写满，平均只填到 50–60%。

```
✗ 09-linian-3.html   填充 50%  底部空 673px  121 字
✗ 06-jiaocheng-2.html 填充 43%  底部空 756px  118 字
```

原因不是不懂规则，是「满不满」靠肉眼估，一忙就放过去。所以量出来：
正文页 44px/lh1.9，每行约 24 字、每行占 84px，可用高度约 1290px ≈ 15 行 ≈ **280–340 字**。
低于 260 字底部一定空。**修法是加真内容，不是调字号撑开。**

```bash
node tools/check-fill.js posts/post-xxx/
```

现在 20 张全部 ✓。

## 违禁词扫描 `redline-scan.sh`

这 9 套 demo 写完第一版跑扫描，**自己就踩了两处**：
