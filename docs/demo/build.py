...
body('02-shishen-2.html', '古人的十种状态 · 01', [
 ...
 '这一点是它和现代那些人格测试最不一样的地方。MBTI 测的是<span class="em">你是谁</span>，'
 '答完题你拿到一个四个字母的标签，走到哪儿都能用。这套测的是'
 '<span class="em">你和外面的东西怎么接上</span>，换一样东西，关系就重算一次。',
 '所以你会看到很奇怪的现象：两个人同一天遇到同一件事，一个觉得是天上掉馅饼，'
 '另一个觉得是麻烦上门。不是他们心态不同，是那件事在各自的坐标里，本来就是两种角色。',
])
...
body('05-baziqa-3.html', '模型测评 · 02', [
 ...
 '两个容易被忽略的处理：一、用<span class="em">宏平均</span>不是总平均——'
 '四届题量不一样，总平均会被题多的那届带偏。二、2021 那届<span class="em">砍掉了</span>，'
 '因为那年没有人类选手成绩，放进去人类那三根柱子就不是同一口径了。',
 '灵伴 <span class="em">37.1%</span>，比在场所有通用大模型都高。'
 '但离人类冠军的 44.4% 还差一截——<span class="em">我们没有赢过人，这个得说实话</span>。'
 '看到有人把这类图做成「AI 已超越人类专家」，那不是我们的数。',
 '还有一句也得说实话：这张图只包含我们本地测过的五个模型。'
 '最新的几个模型在我们另一份榜单上已经反超了灵伴，'
 '而那份榜单里没有灵伴这一行，所以没法画到一起来。要加，得先补测。',
 '<span class="last">数好看不好看是一回事，口径老实不老实是另一回事。'
 '后者更值钱。</span>',
])
...
body('06-jiaocheng-4.html', '报告怎么读 · 03', [
 ...
 '路宽的地方，同样一件事你走过去不费劲；路窄的地方，别人轻松做完的，你得咬着牙。'
 '所以对着这张表读自己那两个角，读出来的应该是两句大白话，'
 '类似「我对气氛敏感」「我手上的活儿一做就烦」——能说成这样，才算读懂了。',
])
...
body('07-dashang-2.html', '灵体大赏 · 01', [
 ...
 '<li><b>不看粉丝数，也不看笔记长短</b>想写两句为什么这颗长这样当然更好，'
 '不写也不影响入选。</li>'
 '</ol>',
...
body('08-jieling-3.html', '节气手记 · 02', [
 ...
 '脾干的活儿就是把吃进去的东西转成能用的力气。两边对的是同一件事。',
 '湿气一重它就转不动，于是第一反应就是不想吃、吃完困、身上发沉——'
 '正好是上面那几样。',
 '所以「最先扛不住的地方」不是硬套出来的，'
 '是长夏配土、土配脾这条对应关系直接推下来的——'
 '这套模型里，每一段时间都有它对应的那个脏，长夏对的就是脾。',
 '顺带能解释另外几件事：为什么这几天特别想吃重口的（转不动就想找刺激），'
 '为什么明明没干什么却累（力气卡在半路没转成），'
 '为什么一喝冰的反而更沉（本来就转不动，再泼一盆凉水）。',
 '<span class="last">这些体感串在一起，指的是同一件事。</span>',
])
...
body('09-linian-2.html', '我们怎么想的 · 01', [
 ...
 '「此人宜出外」下面没有一句为什么。',
 '结论可以背，推理背不了。喂再多古籍进去，模型学会的也只是'
 '「看到这个组合就说这句话」——遇到书里没写过的组合，它就开始编。',
 '所以真正的活儿是：把散在几百年里的判断，一条条还原成'
 '<span class="em">「在什么条件下才成立」</span>。举个我们真拆过的：',
 '古书里有句「此人宜出外」，四个字，下面没有理由。还原出来是这样——'
 '这句成立的前提是：<span class="em">你缺的那一样，在出生地偏少、在别处偏多。</span>'
 '所以它不是一句祝福，是一个带条件的推论。',
 '条件一换，结论就变：你缺的那样要是哪儿都一样少，走出去也补不上；'
 '你要是本来就不缺，出去反而是白折腾。'
 '<span class="last">四个字，还原完是三种情况，古书只写了其中一种。'
 '这活儿没法外包——要做的不是翻译，是替一群不在了的人补上他们省略的那一步。</span>',
])
